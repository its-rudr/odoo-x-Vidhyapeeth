    ...
    <div>
      ...
      <div className="flex gap-3 mb-6">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search drivers..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2" style={{ focusBoxShadow: 'none' }} onFocus={(e) => { e.style.boxShadow = '0 0 0 3px rgba(221, 112, 11, 0.1)'; }} onBlur={(e) => { e.style.boxShadow = ''; }} />
        </div>
      </div>
              ...
              <Card key={d._id} className="p-5 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-2 text-sm text-slate-600 mb-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`flex items-center gap-1 ${expired ? 'font-semibold px-2 py-1 rounded text-white' : 'text-slate-800'}`} style={expired ? { backgroundColor: '#DD700B' } : {}}>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-20 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${d.safetyScore >= 80 ? 'bg-emerald-500' : d.safetyScore >= 50 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${d.safetyScore}%`, backgroundColor: d.safetyScore >= 80 ? '#DD700B' : d.safetyScore >= 50 ? '#ADACA7' : '#DC2626' }} />
                      </div>
                  ...
                  <div className="flex justify-between">
                    <span>Trips</span>
                    <span><span className="font-semibold" style={{ color: '#DD700B' }}>{d.tripsCompleted}</span> completed, <span style={{ color: '#7C7D75' }}>{d.tripsCancelled}</span> cancelled</span>
                  </div>
                </div>
                ...
                {(canEdit || canDelete) && <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
                  ...
                  {canEdit && (d.status !== 'Suspended' ? (
                    <button onClick={() => handleStatusChange(d._id, 'Suspended')} className="flex-1 py-1.5 text-xs font-semibold text-white rounded-lg transition" style={{ backgroundColor: '#DD700B' }}>Suspend</button>
                  ) : (
                    <button onClick={() => handleStatusChange(d._id, 'On Duty')} className="flex-1 py-1.5 text-xs font-semibold text-white rounded-lg transition" style={{ backgroundColor: '#DD700B' }}>Activate</button>
                  ))}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2" style={{ focusBoxShadow: 'none' }} onFocus={(e) => { e.style.boxShadow = '0 0 0 3px rgba(221, 112, 11, 0.1)'; }} onBlur={(e) => { e.style.boxShadow = ''; }} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Email</label>
              <input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2" style={{ focusBoxShadow: 'none' }} onFocus={(e) => { e.style.boxShadow = '0 0 0 3px rgba(221, 112, 11, 0.1)'; }} onBlur={(e) => { e.style.boxShadow = ''; }} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Phone</label>
              <input required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2" style={{ focusBoxShadow: 'none' }} onFocus={(e) => { e.style.boxShadow = '0 0 0 3px rgba(221, 112, 11, 0.1)'; }} onBlur={(e) => { e.style.boxShadow = ''; }} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">License Number</label>
              <input required value={form.licenseNumber} onChange={(e) => setForm({ ...form, licenseNumber: e.target.value })} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2" style={{ focusBoxShadow: 'none' }} onFocus={(e) => { e.style.boxShadow = '0 0 0 3px rgba(221, 112, 11, 0.1)'; }} onBlur={(e) => { e.style.boxShadow = ''; }} />
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-3">
              {['Truck', 'Van', 'Bike'].map((cat) => (
                <label key={cat} className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.licenseCategory.includes(cat)} onChange={() => toggleCategory(cat)} className="rounded border-slate-300" style={{ accentColor: '#DD700B' }} />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="date" value={form.licenseExpiry} onChange={(e) => setForm({ ...form, licenseExpiry: e.target.value })} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2" style={{ focusBoxShadow: 'none' }} onFocus={(e) => { e.style.boxShadow = '0 0 0 3px rgba(221, 112, 11, 0.1)'; }} onBlur={(e) => { e.style.boxShadow = ''; }} />
            </div>
            <div>
